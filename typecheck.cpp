  ...
  private:
    ...
    {   
        ...
        if (strcmp(name, "Main") == 0){
            ...
            return;// in case that Main is inserted for the second time in the next if
        }

        if (! m_st -> insert(name, s))
            // Only report error if there already exists a procesure with the same name in the same scope
...
void dopass_typecheck(Program_ptr ast, SymTab* st)
{
    ...
    // ast is defined as #Program_ptr ast;# in main.cpp
    // Program_ptr is defined as #typedef Program* Program_ptr;# in ast.hpp(automatically generated file by ast.cdef)
    // accept is defined as #void ProgramImpl::accept(Visitor *v) { v->visitProgramImpl(this); }# in ast.cpp
    // What actually be implemented is "typecheck->visitProgramImpl(ast);"
    delete typecheck;
}
